...
4. **Limits** — subscription rate-limit buckets for the current provider (e.g. Anthropic's 5-hour and weekly windows, Codex's primary/secondary limits), each as a label, a remaining-percentage figure, and its own meter bar. Unlike the Context bar, which fills as usage grows, each Limits bar *empties* as the remaining allowance shrinks. The panel is hidden **only** when the provider has no subscription semantics (plain API-key billing). For a subscription provider the panel is always shown: when numbers are missing or stale it renders a short status note (`Waiting for usage data…`, `8m ago`, `refresh failed: …`) instead of disappearing, because a silently vanishing panel hides real failures.
