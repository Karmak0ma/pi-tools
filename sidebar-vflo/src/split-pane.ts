...
export function createSplitPaneController(options: SplitPaneControllerOptions = {}): SplitPaneController {
	...
	const createFullscreenSplitRoot = (originalRoot: Component): Component =>
		new HStack([
			{
				component: originalRoot,
				basis: 0,
				grow: 1,
				shrink: 1,
				// When the dock is hidden, do not retain the docked layout's
				// minimum main width. The renderer can then use narrow terminals
				// normally until the dock is shown again.
				minSize: visibleAt(tui?.terminal.columns ?? 0) ? minimumMain : 1,
			},
			{
