...
export async function contextCommand(ctx: ExtensionCommandContext, runtime: DcpRuntime): Promise<void> {
  ...
  ctx.ui.notify(`pi-dcp context: ${usage?.tokens ?? estimate ?? "unknown"} tokens (${usage?.tokens != null ? "reported" : "heuristic"}); ${active.length} active block(s), savings ${runtime.lastTransform?.savingsTokens ?? "unknown"}, changed prefix ${runtime.lastTransform?.changedPrefix ?? "unknown"}, confidence ${runtime.lastTransform?.confidence ?? "heuristic"}; raw requests ${runtime.contextStats.rawRequests}/${runtime.contextStats.requests}; nudge ${nudgeSummary}. compression: ${readiness?.ready ? "ready" : `unavailable (${readiness?.reason || "unknown"})`}.`, "info");
}
