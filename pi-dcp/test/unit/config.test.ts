...
import { PI_BUILTIN_TOOLS, ALWAYS_PROTECTED_TOOLS } from "../../src/compression/protected.ts";
...
describe("configuration", () => {
  ...
  it("uses conservative opencode-dcp-inspired defaults", () => { expect(defaults.enabled).toBe(true); expect(defaults.compress.permission).toBe("allow"); expect(defaults.nudge).toEqual({ minContextPercent: 35, maxContextPercent: 70, criticalContextPercent: 90, turnsBetweenNudges: 5 }); expect([...PI_BUILTIN_TOOLS]).toEqual(["read", "write", "edit", "bash", "grep", "find", "ls"]); expect([...ALWAYS_PROTECTED_TOOLS]).toEqual(["compress", "write", "edit", "todo"]); expect(defaults.commands.protectedTools).toEqual(["compress", "write", "edit", "todo"]); expect(defaults.compress.protectedTools).toEqual(["todo", "write", "edit"]); expect(defaults.strategies.deduplication.enabled).toBe(true); expect(defaults.strategies.purgeErrors.turns).toBe(4); });
});
