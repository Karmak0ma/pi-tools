...
describe("nudge/cache soak", () => {
  it("keeps the transformed prefix stable across repeated turns", async () => {
    ...
    for (let turn = 0; turn < 50; turn++) {
      ...
      const ctx = { cwd: "/tmp", model: { provider: "test", id: "model", api: "test", contextWindow: 100_000 }, getContextUsage: () => ({ tokens: null, contextWindow: 100_000 }), sessionManager: currentHostSessionManager(entries, `entry-${turn + 1}`), ui: { notify: () => undefined } } as any;
